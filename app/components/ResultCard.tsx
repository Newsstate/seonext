...
export default function ResultCard({ data }:{ data:any }) {
  ...
  const u = data.finalUrl || data.url; // DRY
  ...
  const tabs = [
    ...
    { key:'performance', label:'Performance' }, // <-- put back (since PsiCard is rendered)
  ];
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <section>
              ...
              <div className="kv">
                ...
                <div className="k">HTTP Status</div><div className="v">{data.http?.status ?? '—'}</div>
                <div className="k">X-Robots-Tag</div><div className="v">{data.http?.xRobotsTag || <i>—</i>}</div>
        ...
        <>
          {(data._issues?.length || data._warnings?.length) && (
            <section>
              ...
              <ul className="list-disc pl-6 space-y-1">
                {(data._warnings || [])
                  .filter((w: string) => /(canonical|viewport|lang|robots|render-?blocking|security|mixed\s*content|https?\b)/i.test(w))
