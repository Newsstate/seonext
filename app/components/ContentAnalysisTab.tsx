...
export default function ContentAnalysisTab({ data }: { data: SEOResult }) {
  const ca = (data as any).contentAnalysis as
    | {
        ...
        plagiarism?: any; // not used for output anymore (on-demand)
        seoOptimization: {
          score: number;
          topTerms: string[];
          checks: {
  ...
  const plagRef = useRef<HTMLDivElement>(null);
  const eeatRef = useRef<HTMLDivElement>(null);
  ...
  const handlePlagDone = (out: PlagOut) => {
    setPlagResult(out);
    // reveal & scroll
    requestAnimationFrame(() => plagRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }));
  };

  const handleEEATDone = (out: EEATOut) => {
    setEEATResult(out);
    requestAnimationFrame(() => eeatRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-4">
        ...
        {/* Small status tile for on-demand Plagiarism */}
        <div className="bg-white rounded-xl shadow-sm p-4 space-y-1">
          ...
          {plagResult ? (
            <>
              ...
              <div className="text-xs text-gray-500">Matches: {plagResult.matches.length}</div>
            </>
          ) : (
      ...
      <section className="space-y-4">
        ...
        <PlagiarismCard url={pageUrl} onDone={handlePlagDone} />
        <EEATCard url={pageUrl} onDone={handleEEATDone} />
      </section>
      ...
      <section className="bg-white rounded-xl shadow-sm p-5">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {ca.eat?.hasAuthorByline ? <Chip color="green">Author byline</Chip> : <Chip color="amber">No author byline</Chip>}
          {ca.eat?.hasPublishedDate ? <Chip color="green">Published date</Chip> : <Chip color="amber">No published date</Chip>}
          {ca.eat?.hasUpdatedDate ? <Chip color="green">Updated date</Chip> : <Chip color="gray">No updated date</Chip>}
          {ca.eat?.hasContactOrAbout ? <Chip color="green">Contact/About present</Chip> : <Chip color="amber">Add Contact/About</Chip>}
          {ca.eat?.schemaHints?.hasArticle ? <Chip color="blue">Article schema</Chip> : <Chip color="gray">No Article schema</Chip>}
          {ca.eat?.schemaHints?.hasOrganization ? <Chip color="blue">Organization schema</Chip> : <Chip color="gray">No Organization schema</Chip>}
          {ca.eat?.schemaHints?.hasPerson ? <Chip color="blue">Person schema</Chip> : <Chip color="gray">No Person schema</Chip>}
          {ca.eat?.schemaHints?.hasBreadcrumb ? <Chip color="blue">BreadcrumbList</Chip> : <Chip color="gray">No Breadcrumb</Chip>}
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-4 text-sm text-gray-800">
            ...
            <div>
              {ca.eat?.author?.name || ca.eat?.author?.url ? (
                <>
                  ...
                  {Array.isArray(ca.eat?.author?.sameAs) && (ca.eat?.author?.sameAs?.length || 0) > 0 && (
                    <div className="mt-1 flex flex-wrap gap-2">
                      {(ca.eat?.author?.sameAs || []).slice(0, 8).map((u: string, i: number) => (
                        <a key={i} className="px-2 py-0.5 rounded-full bg-gray-100 text-gray-700" href={u} target="_blank" rel="noreferrer">
                          {hostLabel(u)}
                        </a>
                      ))}
                    </div>
          ...
          <div className="space-y-1">
            ...
            <div>
              ...
              {Array.isArray(ca.eat?.publisher?.sameAs) && (ca.eat?.publisher?.sameAs?.length || 0) > 0 && (
            ...
            <div className="flex flex-wrap gap-2">
              {ca.eat?.policyHints?.hasEditorialPolicy ? <Chip color="blue">Editorial policy</Chip> : <Chip color="gray">No editorial policy</Chip>}
              {ca.eat?.policyHints?.hasCorrectionsPolicy ? <Chip color="blue">Corrections policy</Chip> : <Chip color="gray">No corrections policy</Chip>}
              {ca.eat?.policyHints?.hasFactCheckingPolicy ? <Chip color="blue">Fact-check policy</Chip> : <Chip color="gray">No fact-check policy</Chip>}
              {ca.eat?.policyHints?.hasReviewByline ? <Chip color="green">Reviewed by</Chip> : null}
            </div>

            {Array.isArray(ca.eat?.policyHints?.foundUrls) && (ca.eat?.policyHints?.foundUrls?.length || 0) > 0 && (
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <section className="bg-white rounded-xl shadow-sm p-5 space-y-2">
          ...
          <div className="text-sm grid gap-1">
            ...
            <div>Image alt coverage: <Badge tone={ca.seoOptimization.checks.imageAltCoverage >= 0.7 ? "ok" : "warn"}>{Math.round(ca.seoOptimization.checks.imageAltCoverage * 100)}%</Badge></div>
            <div>Internal links: <Badge>{ca.seoOptimization.checks.internalLinkCount}</Badge></div>
            <div>Top-term density: <Badge tone={ca.seoOptimization.checks.keywordDensityTop > 0.07 ? "warn" : "ok"}>{Math.round(ca.seoOptimization.checks.keywordDensityTop * 1000) / 10}%</Badge></div>
          </div>
          {!!(ca.seoOptimization.notes?.length || 0) && (
            <div className="pt-2 text-xs text-amber-700">
              {ca.seoOptimization.notes.map((n: string, i: number) => <div key={i}>• {n}</div>)}
            </div>
          )}
        </section>
        ...
        <section className="bg-white rounded-xl shadow-sm p-5 space-y-2">
          ...
          {!!(ca.spam.notes?.length || 0) && (
            <div className="pt-2 text-xs text-amber-700">
              {ca.spam.notes.map((n: string, i: number) => <div key={i}>• {n}</div>)}
            </div>
          )}
        </section>

        {/* ↓↓↓ Appear only after clicking Run Check ↓↓↓ */}
        {plagResult && (
          <section ref={plagRef} className="bg-white rounded-xl shadow-sm p-5 space-y-2 md:col-span-2">
        ...
        {eeatResult && (
          <section ref={eeatRef} className="bg-white rounded-xl shadow-sm p-5 md:col-span-2">
            <h4 className="font-semibold border-b pb-2">AI E-E-A-T Verdict</h4>
            <div className="flex items-center justify-between mt-2">
              <div className="text-sm">Verdict: <b>{eeatResult.verdict.toUpperCase()}</b></div>
              {eeatResult.scores && (
                <div className="text-sm">
                  Score: <b>{Math.round(((eeatResult.scores.experience + eeatResult.scores.expertise + eeatResult.scores.authoritativeness + eeatResult.scores.trust) / 4) * 20)}/100</b>
                </div>
              )}
            </div>
            {eeatResult.recommendations?.length ? (
              <ul className="list-disc pl-5 mt-3 text-sm text-gray-800">
                {eeatResult.recommendations.map((r, i) => <li key={i}>{r}</li>)}
              </ul>
            ) : null}
          </section>
        )}
