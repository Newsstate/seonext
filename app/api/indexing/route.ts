...
function originOf(target: string){
...
function parseRobots(txt: string, base: string): RobotsInfo {
  ...
  for (const raw of lines){
    ...
    if (key === 'user-agent'){
      userAgentStar = (val === '*');
    } else if (key === 'allow' && userAgentStar){
      allow.push(val);
    } else if (key === 'disallow' && userAgentStar){
      disallow.push(val);
    } else if (key === 'crawl-delay' && userAgentStar){
      crawlDelay = val;
    } else if (key === 'sitemap'){
      sitemaps.push(normalizeUrl(base, val));
    }
  }
...
async function fetchText(url: string){
  const r = await got(url, {
    http2: true, timeout: { request: 15000 }, retry: { limit: 1 },
...
export async function POST(req: NextRequest){
  try {
    ...
    const guesses = [`${origin}/sitemap.xml`, `${origin}/sitemap_index.xml`, `${origin}/sitemap-index.xml`];
    for (const g of guesses) if (!sitemapCandidates.includes(g)) sitemapCandidates.push(g);

    // fetch & summarize (caps for serverless)
    const parser = new XMLParser({ ignoreAttributes: false, attributeNamePrefix: '' });
    const list: SitemapSummary[] = [];
    let foundPageInSitemap = false;
    let checkedCount = 0;
    let totalUrls = 0;
          ...
          for (const child of arr.slice(0,5)){
            ...
            try {
              ...
              if (cdata?.urlset){
                ...
                const found = urls.some((entry:any)=> {
                  const l = entry?.loc;
                  try { return l && new URL(l).toString() === pageUrl; } catch { return false; }
                });
                if (found) foundPageInSitemap = true;
              }
            } catch {}
          }
        } else if (data?.urlset){
